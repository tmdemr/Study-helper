...
#db연결을 담당할 함스
def getConnection():
    return pymysql.connect(host='localhost', user='pi', password='raspberry',
                           db='suppoter', charset='utf8')
...
def getScheduler(searchDate):
    ...
    sql = "select id, title, start, end, if(allDay = %s,true,false) allDay from my_schedule where to_days(start) >= to_days(%s) and to_days(end) <= to_days(%s) and user_id='" + user_id + "'"
...
def setScheduler(schedule):
    ...
    if not parameter_checker(schedule) :
        ...
        user_id = session['userid']
        sql = "INSERT INTO my_schedule(title, start, end, allDay, user_id) VALUES (%s, %s, %s, %s, '" + user_id + "')"
